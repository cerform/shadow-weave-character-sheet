...
create policy if not exists "asset_categories_select_all" on public.asset_categories
  for select using (true);
create policy if not exists "asset_categories_admin_insert" on public.asset_categories
  for insert with check (has_role(auth.uid(), 'admin'::app_role));
create policy if not exists "asset_categories_admin_update" on public.asset_categories
  for update using (has_role(auth.uid(), 'admin'::app_role));
create policy if not exists "asset_categories_admin_delete" on public.asset_categories
...
create policy if not exists "assets_select_approved_or_admin" on public.assets
  for select using (
    approved = true
    or has_role(auth.uid(), 'admin'::app_role)
  );
create policy if not exists "assets_insert_admin" on public.assets
  for insert with check (has_role(auth.uid(), 'admin'::app_role));
create policy if not exists "assets_update_admin" on public.assets
  for update using (has_role(auth.uid(), 'admin'::app_role));
create policy if not exists "assets_delete_admin" on public.assets
  for delete using (has_role(auth.uid(), 'admin'::app_role));
...
create policy if not exists "map_entities_select_participants" on public.map_entities
  for select using (
    exists (
...
create policy if not exists "map_entities_dm_admin_insert" on public.map_entities
...
create policy if not exists "map_entities_dm_admin_update" on public.map_entities
  for update using (
    exists (
      select 1 from public.battle_maps bm
      join public.game_sessions gs on gs.id = bm.session_id
...
create policy if not exists "map_entities_dm_admin_delete" on public.map_entities
  for delete using (
    exists (
      select 1 from public.battle_maps bm
      join public.game_sessions gs on gs.id = bm.session_id
...
create policy if not exists "map_entities_player_insert_character" on public.map_entities
  for insert with check (
    type = 'character' and owner_id = auth.uid()
  );
create policy if not exists "map_entities_player_update_character" on public.map_entities
  for update using (
    type = 'character' and owner_id = auth.uid()
  );
...
create policy if not exists "public read map-images" on storage.objects
  for select using (bucket_id = 'map-images');
create policy if not exists "public read models" on storage.objects
  for select using (bucket_id = 'models');
create policy if not exists "public read previews" on storage.objects
  for select using (bucket_id = 'previews');

-- Запись только админам
create policy if not exists "admin write map-images" on storage.objects
...
create policy if not exists "admin write models" on storage.objects
...
create policy if not exists "admin write previews" on storage.objects
