...
export const filterSpellsByClassAndLevel = (
  ...
  const { maxLevel } = calculateAvailableSpellsByClassAndLevel(characterClass, characterLevel);
  ...
  return spells.filter(spell => {
    ...
    const validLevel = spell.level <= maxLevel;
    
    return belongsToClass && validLevel;
  });
};
