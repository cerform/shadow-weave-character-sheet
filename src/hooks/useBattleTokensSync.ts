...
export const useBattleTokensSync = (sessionId: string) => {
  ...
  useEffect(() => {
    ...
    // Убеждаемся что канал сессии подключен
    realtimeManager.connectSession(sessionId).catch(console.error);

    const unsubInsert = realtimeManager.onPgChange(sessionId, 'battle_tokens', 'INSERT', (payload) => {
    ...
    const unsubUpdate = realtimeManager.onPgChange(sessionId, 'battle_tokens', 'UPDATE', (payload) => {
    ...
    const unsubDelete = realtimeManager.onPgChange(sessionId, 'battle_tokens', 'DELETE', (payload) => {
